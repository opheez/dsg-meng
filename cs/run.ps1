...
# foreach ($p in $ps) { for ($t = 1; $t -lt ($tmax + 1); $t++) { .\samples\epvs\EpvsMicrobench\bin\Release\net5.0\EpvsMicrobench.exe -t $t -p $p -u ".\results\micro-p$p-throughput.txt"}}

for ($e = $tmax; $e -lt (64 * $tmax); $e = $e * 2) { for ($t = 1; $t -lt ($tmax + 1); $t++) { .\samples\epvs\EpvsMicrobench\bin\Release\net5.0\EpvsMicrobench.exe -t $t -p 1e-4 -e $e  -u ".\results\micro-e$e-throughput.txt"}}
